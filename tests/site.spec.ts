...
for (const width of [360, 390, 430, 1440]) {
  test(`Layout and images at ${width}px`, async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("O preço está");
    ...
    await page.screenshot({
      path: `test-results/proj-${width}.png`,
      fullPage: true,
    });
  });
}
test("Comparison, cases, FAQ, qualification and copy on mobile", async ({
  page,
  context,
}) => {
  ...
  await page
    .getByText("Posso enviar uma imagem de IA ou uma referência?", {
      exact: true,
    })
    .click();
  await expect(
    page.getByText("Sim. Ela ajuda a mostrar o que você imagina.", {
      exact: false,
    }),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "Continuar", exact: true }).click();
  await page
    .getByLabel("Você tem as medidas?")
    .selectOption("Tenho algumas medidas");
  await page
    .getByLabel("Já tem projeto ou planta?")
    .selectOption("Tenho referências ou imagem de IA");
  await page.getByRole("button", { name: "Voltar" }).click();
  await expect(page.getByLabel("Em qual cidade?")).toHaveValue(
    "São José dos Campos, SP",
  );
  await expect(
    page.getByRole("button", { name: "Cozinha", exact: true }),
  ).toHaveAttribute("aria-pressed", "true");
  await page.getByRole("button", { name: "Continuar", exact: true }).click();
  await expect(page.getByLabel("Você tem as medidas?")).toHaveValue(
    "Tenho algumas medidas",
  );
  await page.getByRole("button", { name: "Continuar", exact: true }).click();
  await page.getByRole("button", { name: "Ver minha mensagem" }).click();
  await expect(page.getByLabel("Mensagem para copiar")).toContainText(
    "São José dos Campos, SP",
  );
  await expect(page.getByLabel("Mensagem para copiar")).toContainText(
  ...
  await expect(page.locator(".message-preview").getByRole("status")).toHaveText(
    "Mensagem copiada!",
  );
